...
def build_pairs(bam_for : str = "group1.1.bam", bam_rev : str = "group1.2.bam", bam_for_rescued :str = "group2.1.rescued.bam", bam_rev_rescued : str = "group2.2.rescued.bam", mode : bool = False, output_dir : str = None) -> None:
    ...
    bam_rev : str, optional
        Path to reverse .bam file for the construction of the .pairs equivalent file (non rescued), by default "group1.2.bam"
    ...
    if not mode:
        logger.info(f"Start building pairs file for unique aligned reads.")
        ...
        with open(output_path / "unrescued.pairs", "w") as f_out:
            ...
            for forward_read, reverse_read in zip(bam_for_handler, bam_rev_handler):
                ...
                f_out.write(f"{forward_read.query_name}\t{forward_read.reference_name}\t{forward_read.reference_start+1}\t{reverse_read.reference_name}\t{reverse_read.reference_start+1}\
                            \t{'+' if forward_read.flag == 0 or forward_read.flag == 256 else '-'}\t{'+' if reverse_read.flag == 0 or reverse_read.flag == 256 else '-'}\n")
        ...
        logger.info(f"Start building pairs file for unique and ambiguous reads.")
        ...
        with open(output_path / "rescued.pairs", "w") as f_out:
...
def hicstuff_process(pairs : str = "file.pairs", enzyme : str = "DpnII", genome : str = None,  
                     cpus : int = 8, output_dir : str = None) -> None:
    """
    Take .pairs file and apply hicstuff processing.
    Removal of non informative events, duplicates etc. 
    Gereates bg2 file

    Parameters
    ----------
    pairs : str
        Path to pairs file, by default "file.pairs"
    enzyme
        Enzymes used in the Hi-C protocol. 
    genome : str
        Path to the genome, by default None.     
    output_dir : str, optional
        Path to the folder where to save the filtered files, by default None.    
    """
    ...
    print("enzyme for hicstuff option:")
    print(enzyme)
   
    # application of the hicstuff post processing 
    hicstuff_cmd = f"hicstuff pipeline --start-stage=pairs --enzyme={enzyme} --threads={cpus} --no-cleanup --filter --duplicates --force --genome={genome} {pairs_path}"
    sp.run(hicstuff_cmd, shell=True)
    
    filtered_file_path = output_path / Path(pairs+".filtered")
    rename_cmd = f"gunzip valid_idx_pcrfree.pairs.gz ; mv valid_idx_pcrfree.pairs  {filtered_file_path}"
    sp.run(rename_cmd, shell=True)
    
    logger.info(f"Hicstuff processed files successfully created in {output_path}")
    
def build_matrix(bins : str = "fragments_fixed_sizes.txt", pairs : str = "file.pairs", balance : bool = False, mad_max : int = 5,
                 cpus : int = 8, output_dir : str = None) -> None:
    """
    ...
    output_dir : str, optional
        Path to the folder where to save the cooler matrix file, by default None
    """
    ...
    cool_path = output_path / Path(pairs+".cool")
    
    # creation of the cool file
    cooler_cmd = f"cooler cload pairs -c1 2 -p1 3 -p2 4 -c2 5 {bins_path} {pairs_path} {cool_path}"
    sp.run(cooler_cmd, shell=True)

    # normalisation of the cool file 
    if balance:
        balance_cmd = f"cooler balance --nproc {cpus} --mad-max {mad_max} {cool_path}"
...
def merge_predictions(output_dir : str = None, clean : bool = True, stage = "prediction", cpus : int = 4) -> None:
    """
    ...
    cpus : int, optional
        Number of cpus to use for the merging, by default 4
    """
...
def tidy_folder(output_dir : str = None) -> None:
    ...
    for file in files :
        ...
        elif Path(file).suffix == ".filtered":
                Path(file).rename(output_path / "contacts" / "pairs" / Path(file).name)     
    ...
    for file in plot_files :
        ...
        elif "events" in Path(file).name:
            Path(file).rename(output_path / "plots" / "ps" / Path(file).name)

        # elif Path(file).name.startswith("chr"):
        elif "raw" in Path(file).name or "normalised" in Path(file).name:    
