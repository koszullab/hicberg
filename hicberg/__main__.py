...
@click.option("--patch_small", "-ps", required = False, default = 15, type = int, show_default = True, metavar = "<int>", help = "Size of the small patch for contact density estimation.")
@click.option("--patch_large", "-pl", required = False, default = 45, type = int, show_default = True, metavar = "<int>", help = "Size of the large patch for contact density estimation.")
...
def statistics_cmd(data, mode, kernel_size, deviation,  rate, enzyme, circular, bin_size, output, cpus, blacklist, patch_small, patch_large):
    """
    ...
    if mode in ["full", "density"]:
            hst.generate_density_map2(threads  = cpus, output_dir  = output, patch_small = patch_small , patch_large = patch_large)
    return
