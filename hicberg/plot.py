...
def plot_coverages(bin_size : int = 2000, output_dir : str = None ) -> None:
    ...
    for chromosome in coverage.keys():
        plt.figure()
        plt.plot(coverage[chromosome], label="Smoothed coverage")        
...
def plot_matrix(unrescued_matrix : str = "unrescued.pairs.filtered.cool", rescued_matrix : str = "rescued.pairs.filtered.cool", restriction_map : str = "restriction_map.npy", genome : str = "", vmin : float = 0.0, vmax : float = 3.5, bins : int = 2000, output_dir : str = None) -> None:
    """
    ...
    for i in rescued_matrix.chromnames:
        ...
        masked_matrix1 = np.ma.masked_invalid(np.log10(unrescued_matrix.matrix(balance=False).fetch(i)))
        ...
        ax1.set_title(f"Unrescued map of {i} \n binned at {int(rescued_matrix.binsize / 1000 )}kb",loc="center")
        ...
        masked_matrix2 = np.ma.masked_invalid(np.log10(rescued_matrix.matrix(balance=False).fetch(i)))
        ...
        ax2.set_title(f"Rescued map of {i} \n binned at {int(unrescued_matrix.binsize / 1000 ) }kb",loc="center")
        ...
        plt.savefig(output_path / f"chr_{i}.pdf",format="pdf", dpi=500)
        plt.close()
        
        
        # Plot the normalised matrices
        fig = plt.figure(figsize=(20, 20))
        gs = gridspec.GridSpec(2, 2, height_ratios=[10, 1], width_ratios=[1, 1])
        ...
        divider1 = make_axes_locatable(ax1)
        cax1 = divider1.append_axes("right", size="5%", pad=0.1)
        masked_matrix1 = np.ma.masked_invalid(np.log10(unrescued_matrix.matrix(balance=True).fetch(i)))
        ...
        divider2 = make_axes_locatable(ax2)
        cax2 = divider2.append_axes("right", size="5%", pad=0.1)
        masked_matrix2 = np.ma.masked_invalid(np.log10(rescued_matrix.matrix(balance=True).fetch(i)))
        ...
        plt.savefig(output_path / f"chr_{i}_normalised.pdf",format="pdf", dpi=500)
        plt.close()
