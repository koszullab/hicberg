...
XS = "xs.npy"
COVERAGE1 = "coverage1.npy"  # natural 
COVERAGE = "coverage.npy"    # smoothed 
UNCUTS = "uncuts.npy"
WEIRDS = "weirds.npy"
CIRCLES = "circles.npy"
...
def plot_coverages(bin_size : int = 2000, output_dir : str = None ) -> None:
    """
    Plot coverages along chromosomes
    
    Parameters
    ----------
    bins : int, optional
        Size of the desired bin., by default 2000 bp
    output_dir : str, optional
    ...
    # reload dictionaries
    coverage1 = load_dictionary(output_path / COVERAGE1)
    coverage = load_dictionary(output_path / COVERAGE)

    for chromosome in coverage.keys():
        plt.figure()
        plt.plot(coverage[chromosome], label="Smoothed coverage", color="darkblue")        
        plt.plot(coverage1[chromosome],'o', label="Initial coverage", markersize=1.0, color="cornflowerblue")
        plt.title(f"Coverage along the {chromosome} - bins of {bin_size} bp")
...
def plot_laws(output_dir : str = None) -> None:
    ...
    circles = load_dictionary(output_path / CIRCLES)

    for chromosome in xs.keys():
        
        plt.figure(figsize=(10, 10))
        
        plt.loglog(xs[chromosome], weirds[chromosome], "o", label="++/--", color="royalblue")
        plt.loglog(xs[chromosome], uncuts[chromosome], "o", label="+-", color="salmon")
        plt.loglog(xs[chromosome], circles[chromosome], "o", label="-+", color="limegreen")
        
        plt.title(f"Distribution of weirds, uncuts and circles events across {chromosome}")
        plt.xlabel("Logarithmic binned genomic distances")
        plt.ylabel("Number of events divided by trapezoid area")
        plt.grid()
        plt.legend()
        plt.savefig(output_path / f"events_distribution_{chromosome}.pdf", format = "pdf")
        plt.close()

    logger.info(f"Saved plots of events at : {output_path}")
...
def plot_trans_ps(output_dir : str = None) -> None:
    ...
    # reload dictionaries
    xs = load_dictionary(output_path / XS)
    inter = load_dictionary(output_path / TRANS_PS)
    chrm_sets = product((xs.keys()), repeat=2)

    t_ps = np.zeros((len(xs.keys()) ** 2, 1))
    trans_ps_dictionary = dict()

    for idx, s in enumerate(chrm_sets):
        trans_ps_dictionary[s] = inter.get(s)
        t_ps[idx] = inter.get(s)

    t_ps = t_ps.reshape((len(xs.keys()), (len(xs.keys()))))
    np.fill_diagonal(t_ps, np.nan)

    plt.figure(figsize=(10, 10))
    plt.imshow(np.log(t_ps), cmap="Wistia", interpolation="None")
    plt.colorbar(fraction=0.046)
    plt.xticks(
        np.arange(len(list(xs.keys()))),
        list(xs.keys()),
        rotation="vertical",
    )
    plt.yticks(np.arange(len(list(xs.keys()))),list(xs.keys()))
    
    plt.title("Inter contacts")
    
    plt.savefig(output_path / f"inter_contacts.pdf", format = "pdf")
    plt.close()

    logger.info(f"Saved Inter contacts at : {output_path}")
...
def plot_density(output_dir : str = None) -> None:
    ...
    for chromosome_couple in density_map.keys():
        ...
        plt.figure(figsize=(10, 10))
        plt.imshow(matrix, cmap=cmap, vmin = 0.0, vmax = 2.0)
...
def plot_couple_repartition(forward_bam_file : str = "group2.1.rescued.bam", reverse_bam_file : str = "group2.2.rescued.bam",  output_dir : str = None ) -> None:
    """
    Plot read twins numbers distribution.
    ...
    plt.xlabel("Number of possible paires")
    plt.ylabel("Number of occurences")
    plt.title("Distribution of possible pairs numbers")
    plt.legend()

    plt.savefig(output_path / f"pairs_numbers_distribution.pdf",
        format="pdf",
    )
    plt.close()

    logger.info(f"Saved couple numbers distribution at : {output_path}")

def plot_matrix(unrescued_matrix : str = "unrescued.pairs.filtered.cool", rescued_matrix : str = "rescued.pairs.filtered.cool", 
                restriction_map : str = "restriction_map.npy", genome : str = "", vmin : float = 0.0, vmax : float = 3.5, 
                bins : int = 2000, output_dir : str = None) -> None:
    """
    Plot matrix with additional tracks

    Parameters
    ----------
    unrescued_matrix : str, optional
        Path to the unrescued map file, by default unrescued.pairs.filtered.cool
    rescued_matrix : str, optional
        Path to rescued map file, by default rescued.pairs.cool
    ...
    genome : str, optional
        Path to the genome to digest, by default None
    vmin : float, optional
    ...
    current_cmap = plt.colormaps['afmhot_r']
    current_cmap.set_bad(color='white')    

    # Get the matrices
    unrescued_matrix_cool = load_cooler(output_path /unrescued_matrix)
    rescued_matrix_cool  = load_cooler(output_path /rescued_matrix)

    genome_file = bf.load_fasta(genome, engine="pysam")
    restriction_map = load_dictionary(output_path / restriction_map)
    bins = unrescued_matrix_cool.bins()[:]
    gc_cov = bf.frac_gc(bins[["chrom", "start", "end"]], genome_file)

    chromosomes_couples = list(itertools.combinations_with_replacement(unrescued_matrix_cool.chromnames, 2))
        
    for i,j in chromosomes_couples :
        lower = rescued_matrix_cool.extent(i)[0]
        upper = rescued_matrix_cool.extent(j)[1]

        # coverages
        coverage_unrescued = np.sum(unrescued_matrix_cool.matrix(balance = False).fetch(i,j), axis = 0)
        median_coverage = np.repeat(np.median(coverage_unrescued), coverage_unrescued.shape[0])
        coverage_rescued = np.sum(rescued_matrix_cool.matrix(balance = False).fetch(i,j), axis = 0)
        ...
        masked_matrix1 = np.ma.masked_invalid(np.log10(unrescued_matrix_cool.matrix(balance=False).fetch(i,j)))
        im_unrescued = ax1.imshow(masked_matrix1, vmin = vmin, vmax = vmax, cmap=current_cmap)
        fig.colorbar(im_unrescued, cax=cax1, label="Contacts scores")
        ax1.set_title(f"{str(unrescued_matrix)} map of {i} {j} \n binned at {int(unrescued_matrix_cool.binsize / 1000 )}kb",loc="center")

        ax2 = plt.subplot(gs[1])
        divider2 = make_axes_locatable(ax2)
        cax2 = divider2.append_axes("right", size="5%", pad=0.1)
        masked_matrix2 = np.ma.masked_invalid(np.log10(rescued_matrix_cool.matrix(balance=False).fetch(i,j)))
        im_rescued = ax2.imshow(masked_matrix2, vmin = vmin, vmax = vmax,cmap=current_cmap)
        fig.colorbar(im_rescued, cax=cax2, label="Contact scores")
        ax2.set_title(f"{str(rescued_matrix)} map of {i} {j} \n binned at {int(unrescued_matrix_cool.binsize / 1000 ) }kb",loc="center")

        ax3 = divider1.append_axes("bottom", size="15%", pad=0.5)
        ax3.plot(coverage_unrescued, color="royalblue",linewidth=2.0)
        ax3.plot(median_coverage, linestyle = '--', color = 'black')
        ax3.set_ylabel("Coverage")
        ax3.set_xticks([])
        ax3.set_title('Coverage before reconstruction')
        ax3.set_xlim([0, len(unrescued_matrix_cool.bins().fetch(j))])

        ax4 = divider1.append_axes("bottom", size="15%", pad=0.51)
        ax4.plot(list(gc_cov["GC"][lower:upper]), color="orange")
        ax4.set_ylabel("GC Content")
        ax4.xaxis.set_major_locator(AutoLocator())
        ax4.set_xlim([0, len(unrescued_matrix_cool.bins().fetch(j))])

        ax5 = divider2.append_axes("bottom", size="15%", pad=0.5)
        ax5.plot(coverage_unrescued, label="Before Hicberg", color="royalblue",linewidth=2.0)
        ax5.plot(coverage_rescued, label="After Hicberg", color="limegreen",linewidth=2.0)
        ax5.plot(median_coverage, linestyle = '--', color = 'black')
        ax5.set_title('Coverage after reconstruction')
        ax5.set_xlim([0, len(unrescued_matrix_cool.bins().fetch(j))])
        ax5.set_ylabel("Coverage")
        ax5.legend(loc="center left", bbox_to_anchor=(1, 0.5))
        ax5.set_xticks([])

        ax6 = divider2.append_axes("bottom", size="15%", pad=0.5)
        ax6.plot(list(gc_cov["GC"][lower:upper]), color="orange",linewidth=2.0)
        ax6.set_ylabel("GC Content")
        ax6.set_xlabel("Genomic coordinates")
        ax6.xaxis.set_major_locator(AutoLocator())
        ax6.set_xlim([0, len(unrescued_matrix_cool.bins().fetch(j))])
        
        plt.savefig(output_path / f"{i}_{j}_raw.pdf",format="pdf", dpi=500)
        plt.close()
        
        
        # Plot of the normalised matrices
        fig = plt.figure(figsize=(20, 20))
        gs = gridspec.GridSpec(2, 2, height_ratios=[10, 1], width_ratios=[1, 1])
        ...
        divider1 = make_axes_locatable(ax1)
        cax1 = divider1.append_axes("right", size="5%", pad=0.1)
        masked_matrix1 = np.ma.masked_invalid(np.log10(unrescued_matrix_cool.matrix(balance=True).fetch(i,j)))
        im_unrescued = ax1.imshow(masked_matrix1, cmap=current_cmap)
        fig.colorbar(im_unrescued, cax=cax1, label="Contacts scores")
        ax1.set_title(f"{str(unrescued_matrix)} of {i} {j} \n binned at {int(rescued_matrix_cool.binsize / 1000 )}kb",loc="center")
        ...
        divider2 = make_axes_locatable(ax2)
        cax2 = divider2.append_axes("right", size="5%", pad=0.1)
        masked_matrix2 = np.ma.masked_invalid(np.log10(rescued_matrix_cool.matrix(balance=True).fetch(i,j)))
        im_rescued = ax2.imshow(masked_matrix2,cmap=current_cmap)
        fig.colorbar(im_rescued, cax=cax2, label="Contact scores")
        ax2.set_title(f"{str(rescued_matrix)} of {i} {j} \n binned at {int(unrescued_matrix_cool.binsize / 1000 ) }kb",loc="center")

        ax3 = divider1.append_axes("bottom", size="15%", pad=0.5)
        ax3.plot(coverage_unrescued, color="royalblue")
        ax3.plot(median_coverage, linestyle = '--', color = 'black')
        ax3.set_ylabel("Coverage")
        ax3.set_xticks([])
        ax3.set_title('Coverage before reconstruction')
        ...
        ax4 = divider1.append_axes("bottom", size="15%", pad=0.5)
        ax4.plot(list(gc_cov["GC"][lower:upper]), color="orange")
        ax4.set_ylabel("GC Content")
        ax4.xaxis.set_major_locator(AutoLocator())
        ax4.set_xlim([0, len(unrescued_matrix_cool.bins().fetch(j))])
        ...
        ax5.plot(median_coverage, linestyle = '--', color = 'black')
        ax5.set_title('Coverage after reconstruction')
        ax5.set_xlim([0, len(unrescued_matrix_cool.bins().fetch(str(i)))])
        ax5.set_ylabel("Coverage")
        ax5.legend(loc="center left", bbox_to_anchor=(1, 0.5))
        ax5.set_xticks([])
        ax5.set_xlim([0, len(unrescued_matrix_cool.bins().fetch(j))])
        ...
        plt.savefig(output_path / f"{i}_{j}_normalised.pdf",format="pdf", dpi=500)
        plt.close()
