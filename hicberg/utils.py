...
def detrend(matrix, detectable_bins=None, max_dist=None, 
            smooth=False, fun=np.nanmean, max_val=10):
    """
...
def get_local_density(cooler_file : str = None, 
                      chrom_name : tuple = (None, None), 
                      patch_small : int = 15,
                      patch_large : int = 45) -> np.array:
    """
    Create density map from a Hi-C matrix. 
    Return a dictionary where keys are chromosomes names and values are density maps.
    Density is obtained by getting the local density of each pairwise bin 
    using a gaussian kernel convolution.
    The objective is to capture local structures, such as centromeres and domains. 
    ...
    patch_small : int, optional
        Patch size for the 1rst convolution product by default 15
    patch_large     
        Patch size for the 2d convolution product by default 45
    Returns
    -------
    np.array
        Density contact map filled and smoothed.
    """
    #Load cooler file
    chr1=chrom_name[0]
    chr2=chrom_name[1]

    mat_n = cooler.Cooler(cooler_file).matrix(balance = True).fetch(chr1, chr2)   #
    mat = cooler.Cooler(cooler_file).matrix(balance = False).fetch(chr1, chr2)
    
    # plt.figure(1)
    # plt.imshow(mat_n**.2)
    
    mat = mat.astype(float)
    mat[np.isnan(mat_n).all(axis=1, keepdims=True) | np.isnan(mat_n).all(axis=0, keepdims=True)] = np.nan
    
    print(chr1 ,chr2)
    
    # Detrending of p(s) or inter
    if chr1==chr2:
        mat = csr_matrix(mat)
        mat= mat.tocoo()
        mat=detrend(mat)
        mat=mat.todense()
        mat[mat==0] = 1.0  # here we replace the 0 by 1 to remain more neutral     
        
    if chr1!=chr2:
        mat=mat/np.nanmean(mat)
        mat[mat==0] = 1.0  # here we replace the 0 by 1 to remain more neutral     
        
    # plt.figure(2)
    # plt.imshow(mat, vmin=0, vmax=2.0, cmap="bwr")
    # plt.colorbar()
    
    # automatic determination of patches sizes: TO IMPROVE 
    if patch_small==0 and patch_large == 0:    
        ...
        approx_band_height = np.mean([np.sum(nan_by_row > 0.5)])   # lines with at least 50% of Nan
        approx_band_width  = np.mean([np.sum(nan_by_col > 0.5)])
        
        # print(f"Largeur bandes ~{approx_band_width:.1f}, hauteur bandes ~{approx_band_height:.1f}")
        patch_small = int(np.clip(min(approx_band_height, approx_band_width)/2, 15, 15))
        patch_large = int(np.clip(max(approx_band_height, approx_band_width)*1.5, 30, 80))

        print(f"Patch_small = {patch_small}, patch_large = {patch_large}")
    
    mat_tmp = iterative_fill(mat, patch_size=patch_small, max_iter=5)
    mat_filled = generic_filter(mat_tmp.astype(float), 
                                nanmean_filter, size=patch_large, mode='mirror')
    ...
    # plt.figure(3)
    # plt.imshow(mat_filled, vmin=0, vmax=2.0, cmap="bwr")
    # plt.colorbar()
...
def classify_reads(
    bam_couple: tuple[str, str] = ("1.sorted.bam", "2.sorted.bam"),
    ...
    forward_bam_file_path = output_dir / bam_couple[0]
    reverse_bam_file_path = output_dir / bam_couple[1]
    chromosome_sizes_path = output_dir / chromosome_sizes

    if not forward_bam_file_path.is_file():
        raise IOError(f"Forward alignment file {forward_bam_file_path.name} not found.")
    if not reverse_bam_file_path.is_file():
        raise IOError(f"Reverse alignment file {reverse_bam_file_path.name} not found.")
    if not chromosome_sizes_path.is_file():
        raise IOError(f"Chromosome sizes file {chromosome_sizes_path.name} not found.")
    ...
    fwd_header, rev_header = forward_bam_file.header, reverse_bam_file.header

    # ─── OPTIMISATION : pré-indexer les tailles par tid (entier) ───
    fwd_tid_to_size = [0] * forward_bam_file.nreferences
    rev_tid_to_size = [0] * reverse_bam_file.nreferences
    for ref_name, size in chromosome_sizes_dic.items():
        tid = forward_bam_file.get_tid(ref_name)
        if tid >= 0:
            fwd_tid_to_size[tid] = size
        tid = reverse_bam_file.get_tid(ref_name)
        if tid >= 0:
            rev_tid_to_size[tid] = size

    forward_bam_file_iter = bam_iterator(forward_bam_file_path)
    reverse_bam_file_iter = bam_iterator(reverse_bam_file_path)

    # ─── Création des 6 fichiers de sortie (BAM standard, sans compresslevel) ───
    unmapped_fwd = pysam.AlignmentFile(output_dir / "group0.1.bam", "wb", template=forward_bam_file)
    unmapped_rev = pysam.AlignmentFile(output_dir / "group0.2.bam", "wb", template=reverse_bam_file)
    unique_fwd   = pysam.AlignmentFile(output_dir / "group1.1.bam", "wb", template=forward_bam_file)
    unique_rev   = pysam.AlignmentFile(output_dir / "group1.2.bam", "wb", template=reverse_bam_file)
    multi_fwd    = pysam.AlignmentFile(output_dir / "group2.1.bam", "wb", template=forward_bam_file)
    multi_rev    = pysam.AlignmentFile(output_dir / "group2.2.bam", "wb", template=reverse_bam_file)

    nb_unmapped_couples = nb_multi_mapped_couples = nb_unique_couples = 0
    nb_unmapped_reads_forward = nb_unmapped_reads_reverse = 0
    nb_uniquely_mapped_reads_forward = nb_uniquely_mapped_reads_reverse = 0
    nb_multi_mapped_reads_forward = nb_multi_mapped_reads_reverse = 0

    for forward_block, reverse_block in zip(forward_bam_file_iter, reverse_bam_file_iter):

        # Classification
        unmapped_couple = False
        for r in forward_block:
            if is_unmapped(r):
                unmapped_couple = True
                break
        if not unmapped_couple:
            for r in reverse_block:
                if is_unmapped(r):
                    unmapped_couple = True
                    break
        ...
        if not unmapped_couple:
            for r in forward_block:
                if is_ambiguous(r) or is_poor_quality(r, mapq):
                    multi_mapped_couple = True
                    break
            if not multi_mapped_couple:
                for r in reverse_block:
                    if is_ambiguous(r) or is_poor_quality(r, mapq):
                        multi_mapped_couple = True
                        break

        # Sélection des sorties et compteurs
        if unmapped_couple:
            out_for, out_rev = unmapped_fwd, unmapped_rev
    ...
    # Fermeture
    for fh in (forward_bam_file, reverse_bam_file,
               unmapped_fwd, unmapped_rev, unique_fwd, unique_rev, multi_fwd, multi_rev):
        fh.close()

    logger.info(f"Unmapped couples     : {nb_unmapped_couples}")
    logger.info(f"Unique couples       : {nb_unique_couples}")
    logger.info(f"Multi-mapped couples : {nb_multi_mapped_couples}")
...
def mad_smoothing(vector : np.ndarray[int] = None, window_size : int | str = "auto", nmads : int = 2) -> np.ndarray[int]:
    ...
    Parameters
    ----------
    vector : np.ndarray[int], Data to smooth,
    ...
    nmads : int, optional
        number of median absolute deviation to use
    ...
    if window_size == "auto":
        ...
        window_size = (max_consecutive_nans(imputed_nan_data) ) + 1
...
def mad_smoothing2(y : np.ndarray[int] = None, 
                   outlier_window : int | str = "auto", 
                   n_mad_local=3.0, 
                   n_mad_global=2.0, 
                   envelope_window=51, 
                   percentile=90, 
                   smooth_window=21) -> np.ndarray[int]:

    """
    To interpolate, extrapolate and smooth the coverage signal. 
    1. Remove low outliers (local + global)
    2. Fit the upper envelope using a sliding high percentile
    3. Slightly smooth
    ...
    y : np.ndarray[int], optional
        Data to smooth
    outlier_window : int or str, optional
        Size of the window to perform outlier removal, by default "auto"
    n_mad_local: int, optional
        number of median absolute deviation to use, by default 3 for local
    n_mad_global: int, optional
        number of median absolute deviation to use, by default 2 for global
    envelope_window: int, optional
        window size for the percentile
    percentile: int, optional
        75 = mild, 90 = follows the high more closely, 100 = maximum
    ...
    threshold = np.median(y) - n_mad_global * mad
    # threshold = 0
    imputed_nan_data = np.where(y <= threshold, np.nan, y)

    if outlier_window == "auto":
        # due to centered window, selected windows for rolling mean is :
        # [window_size / 2 <-- center_value --> window_size / 2]
        outlier_window = (max_consecutive_nans(imputed_nan_data) ) + 1

    print("window size to remove outlier for coverage smoothing")
    print(outlier_window)
    
    # --- ÉTAPE 1 : outliers bas ---
    local_median = median_filter(y, size=outlier_window, mode="reflect")
    local_mad = median_filter(np.abs(y - local_median), size=outlier_window, mode="reflect")
    local_mad = np.where(local_mad == 0, 1e-10, local_mad)
    lower_local = local_median - n_mad_local * local_mad
    
    global_median = np.median(y)
    deviations = global_median - y[y <= global_median]
    global_mad = np.median(deviations) if len(deviations) > 0 else 1e-10
    lower_global = global_median - n_mad_global * global_mad
    
    mask = (y >= lower_local) & (y >= lower_global)
    y_clean = y.copy()
    y_clean[~mask] = np.nan
