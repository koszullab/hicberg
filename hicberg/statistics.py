...
XS = "xs.npy"
COVERAGE_DICO1= "coverage1.npy"   # without smooth
COVERAGE_DICO = "coverage.npy"    # with smooth
D1D2 = "d1d2.npy"
UNCUTS = "uncuts.npy"
WEIRDS = "weirds.npy"
CIRCLES = "circles.npy"
...
def generate_coverages(genome : str = None, bin_size : int = 2000, forward_bam_file : str = "group1.1.bam", reverse_bam_file : str = "group1.2.bam", output_dir : str = None) -> None:
    """
    Take a genome and both forward and reverse bam files for unambiguous group 
    and return a dictionary containing the coverage natural and smoothed for 
    each chromosome.
    ...
    output_dir : str, optional
        Path to the folder where to save the coverage files, by default None, by default None
    """        
    ...
    # Coverages
    natural_coverages = {seq_name : coverage for seq_name, coverage in genome_coverages.items()}
    smoothed_coverages = {seq_name : hut.mad_smoothing2(coverage) for seq_name, coverage in genome_coverages.items()}

    np.save(output_path / COVERAGE_DICO1, natural_coverages)
...
def generate_density_map2(cooler_file : str = "unrescued.pairs.cool",
                          threads : int = 2, 
                          output_dir : str = None,
                          patch_small : int = 15, 
                          patch_large : int = 45) -> None:
    """
    ...
    results = pool.map(partial(hut.get_local_density, str(matrix_path), patch_small=patch_small, patch_large=patch_large),chromosomes_couples)
...
def get_trans_ps(read_forward : pysam.AlignedSegment, 
                 read_reverse : pysam.AlignedSegment, 
                 trans_ps : dict) -> float:
    """
    Parameters
...
def compute_propensity(read_forward : pysam.AlignedSegment, read_reverse : pysam.AlignedSegment, restriction_map : dict = None, xs : dict = None, 
                       ...
                       coverage : dict = None, bin_size : int = 2000, d1d2 : dict = None, density_map : dict = None,  mode : str = "standard",
                       rng_numpy: np.random.Generator= None) -> float:
    """
...
# ============================================================================
# 1. FACTORY : crée la fonction de calcul ad hoc selon le mode (hors boucle)
# ============================================================================

def _make_propensity_calculator(
    mode: str,
    restriction_map,
    xs,
    weirds,
    ...
    """
    Retourne une fonction (fwd, rev) -> float spécialisée pour le mode donné.
    Zéro comparaison de string à l'exécution.
    """
    ...
    if mode == "coverage":
        def calc(fwd, rev, bin_size=bin_size, circular=circular):
            return get_coverages(fwd, rev, coverage, bin_size=bin_size)
        return calc
...
# ============================================================================
# 2. TIRAGE PONDÉRÉ (inchangé / déjà optimisé)
# ============================================================================

def draw_read_couple_flat(propensities: np.ndarray, rng: np.random.Generator) -> int:
    """
    Tirage pondéré sur un array PLAT déjà pré-alloué.
    Nettoie NaN, Inf et valeurs négatives, puis tire selon la distribution.
    Retourne l'index plat.
    """
    # 1. Remplace NaN / +Inf / -Inf par 0 (en place)
    np.nan_to_num(propensities, copy=False, nan=0.0, posinf=0.0, neginf=0.0)

    # 2. Remplace les valeurs négatives par 0 (en place)
    np.clip(propensities, 0, None, out=propensities)

    # 3. Somme et tirage
    total = propensities.sum(dtype=np.float64)

    if total > 0.0:
        pk = propensities / total
        return int(rng.choice(propensities.shape[0], p=pk))
    else:
        # Si tout est nul : tirage uniforme
        return int(rng.integers(propensities.shape[0]))


# ============================================================================
# 3. WORKER POUR LE POOL D'INDICES PLATS
# ============================================================================

def _propensity_from_idx(
    idx: int,
    n_rev: int,
    forward_block,
    reverse_block,
    ...
    """Décompose l'index plat et calcule la propensity."""
    i = idx // n_rev
    j = idx % n_rev
    return compute_fn(forward_block[i], reverse_block[j])


# ============================================================================
# 4. FONCTION PRINCIPALE RÉÉCRITE
# ============================================================================
...
def reattribute_reads(
    ...
    xs: str = "xs.npy",
    weirds: str = "weirds.npy",
    uncuts: str = "uncuts.npy",
    circles: str = "circles.npy",
    circular: str = "",
    trans_ps: str = "trans_ps.npy",
    coverage: str = "coverage.npy",
    bin_size: int = 2000,
    d1d2: str = "d1d2.npy",
    density_map: str = "density_map.npy",
    mode: str = "standard",
    output_dir: str = None,
    rng_numpy: np.random.Generator = None
) -> None:

    if rng_numpy is None:
        rng_numpy = np.random.default_rng(seed=42)

    output_path = Path(getcwd()) if output_dir is None else Path(output_dir)

    # --- Chargement des dicos (inchangé) ---
    xs_dict       = hio.load_dictionary(output_path / xs)
    uncuts_dict   = hio.load_dictionary(output_path / uncuts)
    circles_dict  = hio.load_dictionary(output_path / circles)
    weirds_dict   = hio.load_dictionary(output_path / weirds)
    trans_ps_dict = hio.load_dictionary(output_path / trans_ps)
    coverage_dict = hio.load_dictionary(output_path / coverage)

    d1d2_data    = None
    density_data = None
    if mode in ("d1d2",):          # tuple plus rapide que liste pour le test d'appartenance
        d1d2_data = hio.load_dictionary(output_path / "d1d2.npy")
    elif mode in ("density", "full"):
        density_data = hio.load_dictionary(output_path / "density_map.npy")
    ...
    # --- ✅ CRÉATION UNIQUE DU CALCULATEUR SPÉCIALISÉ ---
    _compute_propensity = _make_propensity_calculator(
        mode=mode,
    ...
    max_workers = 2  # À ajuster selon le relâchement du GIL par les fonctions helper
    use_threading = max_workers > 1
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for forward_block, reverse_block in zip(forward_generator, reverse_generator):
            ...
            # --- Vérification d'appariement UNE SEULE FOIS par bloc ---
            if __debug__:
                if n_for and n_rev:
                    ref_name = forward_block[0].query_name
                    # assert rapide, désactivé avec python -O
                    assert all(r.query_name == ref_name for r in forward_block), "Forward block hétérogène"
                    assert all(r.query_name == ref_name for r in reverse_block), "Reverse block hétérogène"

            # --- Pré-allocation du tableau de propensities ---
            propensities = np.empty(n_comb, dtype=np.float64)

            # --- Calcul : on choisit entre threadé ou séquentiel selon la taille ---
            # Seuil empirique : en dessous de 5000 éléments, le pool coûte plus cher qu'il ne rapporte
            if use_threading and n_comb > 5000:
                _worker = partial(
                    _propensity_from_idx,
                    n_rev=n_rev,
                    forward_block=forward_block,
                    reverse_block=reverse_block,
                    compute_fn=_compute_propensity
                )
                # Remplissage par index pour garder l'ordre
                for k, val in enumerate(executor.map(_worker, range(n_comb))):
                    propensities[k] = val
            else:
                # Boucle pure Python sans overhead de pool (souvent plus rapide pour les petits n_comb)
                for idx in range(n_comb):
                    i = idx // n_rev
                    j = idx % n_rev
                    propensities[idx] = _compute_propensity(forward_block[i], reverse_block[j], bin_size=bin_size, circular=circular)

            # --- Tirage pondéré ---
            selected_flat_idx = draw_read_couple_flat(propensities, rng_numpy)

            # --- Récupération O(1) du couple ---
            i_sel = int(selected_flat_idx // n_rev)
            j_sel = int(selected_flat_idx % n_rev)
            selected_read_forward = forward_block[i_sel]
            selected_read_reverse = reverse_block[j_sel]
