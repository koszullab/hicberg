...
def pipeline(
    ...
    #     logger.error("bedtools is not available on your system, install it \
    #                  if you want to use omics mode.")
    #     # raise ValueError("bedtools is not available on your system.")
    
    # if not check_tool('bedGraphToBigWig'):
    #     logger.error("bedGraphToBigWig is not available on your system, install it \
    #                  if you want to use omics mode.")
    #     # raise ValueError("bedGraphToBigWig is not available on your system.")
        
    if not check_tool('deeptools'):
        logger.error("deeptools is not available on your system, install it \
                     if you want to use omics mode.")
        # raise ValueError("bedGraphToBigWig is not available on your system.")   
    ...
    if start_stage not in ["fastq", "bam", "groups", "build", "stats", "rescue", "final"]:
        ...
        raise IOError(f"Start_stage {start_stage} must be: fastq bam groups build stats rescue final.")
        
    if exit_stage not in ["None", "bam", "groups", "build", "stats", "rescue", "final"]:
        logger.error(f"Exit_stage {exit_stage} must be: None bam groups build stats rescue final.")
        raise IOError(f"Exit_stage {exit_stage} must be: None bam groups build stats rescue final.")
            
    if fq_for == fq_rev :
        logger.error(f"{fq_for} and {fq_rev} files must be different.")
        raise IOError(f"{fq_for} and {fq_rev} files must be different.")
    ...
    if start_stage_n  < 1 : 
        ...
        if index is None:
            index = hal.hic_build_index(genome = genome, output_dir = output_folder, 
    ...
    if start_stage_n < 4:
        ...
        if mode in ["density", "full"]:
            hst.generate_density_map2(cooler_file = "unrescued.pairs.cool",
                                      threads  = cpus,
                                      output_dir  = output_folder,
                                      patch_small = 15, 
                                      patch_large = 45)
    ...
    if start_stage_n  <= 5:
        ...
        restriction_map = hio.load_dictionary(Path(output_folder) / "restriction_map.npy")

        # Reattribute reads in parallel, heart of the algo !
    ...
    if start_stage_n  <= 6:
        ...
        if hicstuff_process_bool == "True":
            p5 = Process(target = hpl.plot_matrix, 
                         kwargs = dict(unrescued_matrix = "unrescued.pairs.filtered.cool", 
                                       rescued_matrix = "rescued.pairs.filtered.cool", 
                                       genome = genome, output_dir = output_folder))
        else :
            p5 = Process(target = hpl.plot_matrix, 
                         kwargs = dict(unrescued_matrix = "unrescued.pairs.cool", 
        ...
        if mode in ["d1d2"]:   
            hpl.plot_d1d2(output_dir = output_folder)
            
        if mode in ["density", "full"]:
            hpl.plot_density(output_dir = output_folder)
