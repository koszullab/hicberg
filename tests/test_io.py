...
def test_build_pairs(temporary_folder, hic_build_index, test_hic_align, test_hic_view, hic_sort, classify_reads):
    """
...
@pytest.fixture
def build_matrix(temporary_folder, get_bin_table, hic_build_index, test_hic_align, test_hic_view, hic_sort, classify_reads, test_build_pairs):
    """
...
@pytest.mark.skip(reason="legacy test - to be rewritten")
def test_load_dictionary(get_chromosomes_sizes):
    """
    Test if the dictionary saved in .npy format is correctly loaded.
    """
    dictionary = hio.load_dictionary(dictionary = get_chromosomes_sizes)

    assert DICT_FIRST_KEY == list(dictionary.keys())[0]
    assert DICT_FIRST_SIZE == list(dictionary.values())[0]

@pytest.mark.skip(reason="legacy test - to be rewritten")
def test_load_cooler(build_matrix):

    matrix = hio.load_cooler(matrix = build_matrix)

    assert isinstance(matrix, cooler.Cooler)

@pytest.mark.skip(reason="legacy test - to be rewritten")
def test_merge_predictions(temporary_folder, chunk_bam):
