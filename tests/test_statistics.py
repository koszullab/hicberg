...
#from.test_utils import classify_reads, get_chromosomes_sizes, get_bin_table, chunk_bam
#from .test_align import hic_build_index, test_hic_align, test_hic_view, hic_sort
#from.test_io import build_matrix, test_build_pairs
...
@pytest.fixture
def generate_xs(temporary_folder):
    """
...
@pytest.fixture
def log_bin_genome(temporary_folder):
    """
    Test if the log binning of the genome is correctly done .
    """
    temp_dir_path = Path(temporary_folder)
...
def test_attribute_xs(generate_xs):
    """
...
@pytest.fixture
def get_dist_frags(temporary_folder, get_restriction_map_mono):
    """
    Test if the distance between restriction sites is correctly computed.
    """
    temp_dir_path = Path(temporary_folder)
    
    hst.get_dist_frags(genome = GENOME, restriction_map = get_restriction_map_mono, circular = CIRCULAR, rate = SUBSAMPLE_RATE, output_dir = temp_dir_path)
...
@pytest.fixture
def generate_trans_ps(temporary_folder, get_restriction_map_mono, build_matrix):
    """
    Test if the transchromosomal P(s) are correctly generated.
    """
    temp_dir_path = Path(temporary_folder)
    hst.generate_trans_ps(matrix = build_matrix, output_dir = temp_dir_path)
...
@pytest.fixture
def generate_coverages(temporary_folder, classify_reads):
    """
    Test if the coverages are correctly generated.
    """
    temp_dir_path = Path(temporary_folder)
    hst.generate_coverages(genome = GENOME, forward_bam_file = classify_reads[0], reverse_bam_file = classify_reads[1], bin_size = BINS, output_dir = temp_dir_path)
...
@pytest.fixture
def generate_d1d2(temporary_folder, classify_reads, get_restriction_map_mono):
    """
    Test if the d1d2 law is correctly generated.
    """
    temp_dir_path = Path(temporary_folder)
    hst.generate_d1d2(forward_bam_file = classify_reads[0], reverse_bam_file = classify_reads[1], restriction_map = get_restriction_map_mono, output_dir = temp_dir_path)
...
@pytest.fixture
def get_patterns(temporary_folder, classify_reads, log_bin_genome, get_dist_frags):
    """
    Test if the patterns are correctly generated.
    """
    temp_dir_path = Path(temporary_folder)
    hst.get_patterns(forward_bam_file = classify_reads[0], reverse_bam_file = classify_reads[1], xs = log_bin_genome, output_dir = temp_dir_path)
...
@pytest.mark.skip(reason="legacy test - to be rewritten")
def test_get_pair_ps(temporary_folder, log_bin_genome, get_patterns):
    """
    ...
    weirds_dictionary_path, uncuts_dictionary_path, loops_dictionary_path = get_patterns
    
    xs = hio.load_dictionary(log_bin_genome)
...
@pytest.mark.skip(reason="legacy test - to be rewritten")
def test_get_trans_ps(generate_trans_ps):
    """
    ...
    trans_ps = hio.load_dictionary(generate_trans_ps)
...
@pytest.mark.skip(reason="legacy test - to be rewritten")
def test_get_coverages(generate_coverages):
    """
    Test if the get_coverages function returns the right coverage regarding a pair of reads.
    """
    ...
    coverages = hio.load_dictionary(generate_coverages)

    cover = hst.get_coverages(read_forward = read_forward, read_reverse = read_reverse, coverage = coverages,  bin_size = BINS)

    assert cover > 0

@pytest.mark.skip(reason="legacy test - to be rewritten")
def test_get_d1d2(get_restriction_map_mono, generate_d1d2):
    """
    ...
    d1d2_law = hio.load_dictionary(generate_d1d2)

    d1d2 = hst.get_d1d2(read_forward = read_forward, read_reverse = read_reverse, restriction_map = get_restriction_map_mono, d1d2 = d1d2_law)
...
@pytest.mark.skip(reason="legacy test - to be rewritten")
def test_compute_propensity(get_restriction_map_mono, log_bin_genome, get_patterns, generate_trans_ps, generate_coverages, generate_d1d2, generate_density_map):
    """
    ...
    trans_ps = hio.load_dictionary(generate_trans_ps)
    coverage = hio.load_dictionary(generate_coverages)
    d1d2 = hio.load_dictionary(generate_d1d2)
    density_map = hio.load_dictionary(generate_density_map)

    restriction_map = get_restriction_map_mono
    propensity = hst.compute_propensity(read_forward = read_forward, read_reverse = read_reverse, restriction_map = restriction_map, xs = xs, weirds = weirds, uncuts = uncuts, loops = loops, trans_ps = trans_ps, coverage = coverage, bin_size = BINS, d1d2 = d1d2, density_map = density_map, mode = MODE)
...
@pytest.mark.skip(reason="legacy test - to be rewritten")
def test_draw_read_couple(get_restriction_map_mono, log_bin_genome, get_patterns, generate_trans_ps, generate_coverages, generate_d1d2, generate_density_map):
    """
    ...
    for couple in all_couples:
        ...
        propensity =  hst.compute_propensity(read_forward = couple[0], read_reverse = couple[1], restriction_map = restriction_map, xs = xs, weirds = weirds, uncuts = uncuts, loops = loops, trans_ps = trans_ps, coverage = coverage, bin_size = BINS, d1d2 = d1d2, density_map = density_map, mode = MODE)
...
@pytest.fixture
def reattribute_reads(temporary_folder, classify_reads, get_restriction_map_mono, log_bin_genome, get_patterns, generate_trans_ps, generate_coverages, generate_d1d2):
    """
    ...
    trans_ps = generate_trans_ps
    coverage = generate_coverages
    d1d2 = generate_d1d2

    restriction_map = get_restriction_map_mono

    forward_bam_file, reverse_bam_file = str(classify_reads[2]), str(classify_reads[3])

    hst.reattribute_reads(reads_couple = (forward_bam_file, reverse_bam_file), restriction_map = restriction_map, xs = xs, weirds = weirds, uncuts = uncuts, loops = loops, trans_ps = trans_ps, coverage = coverage, bin_size = BINS, d1d2 = d1d2, mode = MODE, output_dir = temp_dir_path)
...
@pytest.fixture
def generate_density_map(temporary_folder, get_bin_table, hic_build_index, test_hic_align, test_hic_view, hic_sort, classify_reads, test_build_pairs):
    """
...
@pytest.mark.skip(reason="legacy test - to be rewritten")
def test_get_density(temporary_folder, generate_density_map):
    """
    Test if the get_d1d2 function returns the right d1d2 regarding a pair of reads.
    """
