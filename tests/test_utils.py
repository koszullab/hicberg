...
def _test_get_chromosomes_sizes(temporary_folder):
    """
...
def test_get_bin_table(temporary_folder, test_get_chromosomes_sizes):
    ...
    """
    temp_dir_path = Path(temporary_folder)
    hut.get_bin_table(chrom_sizes_dict = test_get_chromosomes_sizes, bin_size = 2000,  output_dir = temp_dir_path)
...
def test_is_ambiguous():
    ...
    duplicateness = hut.is_ambiguous(read_duplicated)
...
def _test_classify_reads(temporary_folder, hic_sort, test_get_chromosomes_sizes):
    """
    Test if the creation of the different bam files per group is correctly performed.
    """

    temp_dir_path = Path(temporary_folder)
    hut.classify_reads(bam_couple =  (hic_sort[0], hic_sort[1]) , chromosome_sizes = test_get_chromosomes_sizes, mapq = MIN_READ_MAPQ,  output_dir = temp_dir_path)
...
def _test_chunk_bam_classification(temporary_folder, hic_sort):
    """
    Test if the chunk bam function is correctly chunking a couple of bam file (classification stage).
    """

    temp_dir_path = Path(temporary_folder)
    forward_bam_file, reverse_bam_file = str(hic_sort[0]), str(hic_sort[1])
...
def _test_chunk_bam(temporary_folder, test_classify_reads):
    """
